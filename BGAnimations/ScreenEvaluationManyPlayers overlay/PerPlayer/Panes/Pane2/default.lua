...
af.InitCommand=function(self) self:xy(-80, 30) end
...
for i, column in ipairs( cols ) do
		...
		if gmods.TimingWindows[j] or j==#rows then
			...
			af[#af+1] = LoadFont("Common Normal")..{
				...
				InitCommand=function(self)
					self:xy(_x, j*rowHeight + 12)
						:zoom(0.9)
					if j == #rows then miss_bmt = self end

					self:diffuse( SL.JudgmentColors[SL.Global.GameMode][j] )
				end
			}
		end
	end
	...
	if track_missbcheld then
		...
		af[#af+1] = LoadFont("Common Normal")..{
			...
			InitCommand=function(self)
				self:xy(_x + 24, 6*rowHeight + 14):zoom(0.65):halign(1)
			end,
			OnCommand=function(self)
